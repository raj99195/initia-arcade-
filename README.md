...
Player plays game
      ↓
Unity/HTML5 game calls: arcade_gameOver(finalScore)
      ↓
arcade-sdk.js sends postMessage(GAME_OVER) to platform
      ↓
React frontend receives GAME_OVER event
      ↓
Shows "Writing on chain..."
      ↓
    [Auto-sign ON?]
   YES ↙         ↘ NO
submitTxBlock    requestTxBlock
(no popup)       (wallet popup)
      ↓                ↓
  Silent tx       User approves
      ↓                ↓
      └──────┬──────────┘
             ↓
    TX confirmed on initia-arcade-1
             ↓
    Show TX hash + "✓ Score on-chain!"
             ↓
    mint_tokens → ARCADE in player wallet
```
...
Creator connects wallet
      ↓
init_creator() → on-chain creator account
      ↓
Fill game form: name, iframe URL, reward rate
      ↓
register_game() → on-chain pending game
      ↓
Admin calls approve_game() → game goes live
      ↓
Players discover game in library
      ↓
Player plays → GAME_OVER → submit_score()
      ↓
Platform calculates:
  playerReward  = rewardRate × 80%
  creatorReward = rewardRate × 20%
      ↓
mint_tokens(player, playerReward)   ← ARCADE to player
mint_tokens(creator, creatorReward) ← ARCADE to creator
      ↓
Both balances update on-chain
Creator dashboard shows live earnings
```
